...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.Universe Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Universe,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Universe,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    addon_log("SKindex")
    addDir('[B][COLOR lime]SEARCH MOVIES[/B][/COLOR]','http://tvbstreamz.net/addonmenus/home.xml',41,'' ,  FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Universe,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('TVB Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('TVB Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Universe,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(TVB ,Is Finding Your Link - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('TVB','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('TVB', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        headers = dict({'User-Agent':'Mozilla/5.0 (Windows NT 6.3; rv:33.0) Gecko/20100101 Firefox/33.0','Referer': 'http://TVB .org/','Accept-Encoding':'gzip, deflate','Content-Type': 'application/json;charset=utf-8','Accept': 'application/json, text/plain, */*'})
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(TVB,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(TVB ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(TVB,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
